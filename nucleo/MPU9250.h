...
enum Mscale { MFS_14BITS = 0, MFS_16BITS }; // 0.6 mG per LSB, 0.15 mG per LSB
...
public:
  ...
  void initMPU9250() {
    ...
    // enable I2C_BYPASS_EN so additional chips join I2C bus and all can be controlled by the Arduino as master
  ...
  //}}}
  //{{{
  // Similar Madgwick but uses proportionalintegral filtering on error between estimated reference vectors and  measured ones
...
private:
  ...
  void writeByte (uint8_t address, uint8_t subAddress, uint8_t data) {
    ...
    data_write[1] = data;
    if (HAL_I2C_Master_Transmit (&I2cHandle, address, data_write, 2, 10000) != HAL_OK)
      printf ("writeByte  tx error\n");
    }
  //}}}
  //{{{
  char readByte (uint8_t address, uint8_t subAddress) {
    ...
    if (HAL_I2C_Master_Transmit (&I2cHandle, address, data_write, 1, 10000) != HAL_OK)
      printf ("writeByte  tx error\n");
    if (HAL_I2C_Master_Receive (&I2cHandle, address, data, 1, 10000) != HAL_OK)
      printf ("m2950init id rx error\n");
  ...
  void readBytes (uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest) {
    ...
    if (HAL_I2C_Master_Transmit (&I2cHandle, address, data_write, 1, 10000) != HAL_OK)
      printf ("writeByte  tx error\n");
    if (HAL_I2C_Master_Receive (&I2cHandle, address, data, count, 10000) != HAL_OK)
      printf ("m2950init id rx error\n");
